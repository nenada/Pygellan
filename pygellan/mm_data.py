"""
Library for reading micro-manager data sets
"""
import os
import mmap
import numpy as np
...
class _MMMultipageTiffReader:
    #
    # Class corresponding to a single multipage tiff file in a Micro-Manager dataset.
    ...
    def __init__(self, tiff_path):
        ...
        # create memory mapped file representation
...
class _MMDataSetReader:
    ...
    def __init__(self, path):
        ...
        for tiff in tiff_names:
            reader = _MMMultipageTiffReader(tiff)
...
class MMDataset:
    """
    Class that opens a Micro-manager dataset.
    """
            ...
            for t in self.frame_indices:
                for z in self.z_indices :
                    for c in self.channel_indices:
                        if z in self.c_z_t_p_tree[c] and t in self.c_z_t_p_tree[c][z] and p in self.c_z_t_p_tree[c][z][t]:
